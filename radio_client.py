...
from PySide6.QtCore import Qt, QSize, QTimer, Signal, Slot, QEvent, QPointF, QRectF, QObject
...
class TacticalPlotter(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for cs, pos in self.member_positions.items():
            ...
            # If highlighted, draw a pulsing dashed orange target ring
            is_highlighted = False
            win = self.window()
            if win and hasattr(win, '_highlighted_member'):
                if cs.upper() == win._highlighted_member.upper():
                    is_highlighted = True
            ...
            if is_highlighted:
                ...
                # Outer dashed ring
                painter.setPen(QPen(QColor("#f97316"), 1.5, Qt.DashLine))
                painter.drawEllipse(QPointF(px, py), 16, 16)
                # Inner solid ring
                painter.setPen(QPen(QColor("#ea580c"), 1.0))
                painter.drawEllipse(QPointF(px, py), 12, 12)
                # Crosshair ticks
                painter.setPen(QPen(QColor("#f97316"), 1.0))
                painter.drawLine(int(px - 20), int(py), int(px - 16), int(py))
                painter.drawLine(int(px + 16), int(py), int(px + 20), int(py))
                painter.drawLine(int(px), int(py - 20), int(px), int(py - 16))
                painter.drawLine(int(px), int(py + 16), int(px), int(py + 20))
                painter.restore()
...
class ClientAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/status':
            if not self.client:
                ...
                self.wfile.write(json.dumps({"error": "Client not initialized"}).encode('utf-8'))
                return
            ...
            try:
                ...
                curr_channel = cfg.current_channel
                channel_name = "N/A"
                frequency = "0.000 MHz"
                encryption = "PLAIN"
                eccm = "NONE"
                
                if curr_channel in self.client.mgr.channels:
                    ch_info = self.client.mgr.channels[curr_channel]
                    channel_name = ch_info.name
                    try:
                        freq_val = float(ch_info.frequency.split(" ")[0])
                        frequency = f"{freq_val:.3f} MHz"
                    except:
                        frequency = ch_info.frequency
                    encryption = "AES-256" if ch_info.encrypted else "PLAIN"
                    eccm = getattr(ch_info, 'eccm_mode', 'NONE').upper()

                channels_map = {}
                try:
                    channels_map = {str(ch_id): ch.name for ch_id, ch in list(self.client.mgr.channels.items())}
                except Exception:
                    pass

                active_channels = []
                stamps = getattr(self.client, '_channel_activity_stamps', {})
                now = time.time()
                for ch_id, last_ts in list(stamps.items()):
                    if now - last_ts < 2.0:
                        active_channels.append(ch_id)

                members_list = []
                my_lat, my_lon, _ = self.client.mgr.get_position()
                
                nav_bearing = 0.0
                nav_range_km = 0.0
                if (my_lat != 0.0 or my_lon != 0.0) and (self.client._nav_lat != 0.0 or self.client._nav_lon != 0.0):
                    nav_bearing = self.client._calculate_bearing(my_lat, my_lon, self.client._nav_lat, self.client._nav_lon)
                    nav_range_km = self.client._haversine(my_lat, my_lon, self.client._nav_lat, self.client._nav_lon)

                for cs, pos_data in list(self.client._member_positions.items()):
                    lat, lon, alt, ts = pos_data
                    age = now - ts
                    stale = age > 60
                    dist_km = -1.0
                    if (my_lat != 0 or my_lon != 0) and lat != 0 and lon != 0:
                        dist_km = self.client._haversine(my_lat, my_lon, lat, lon)
                    members_list.append({
                        "callsign": cs.upper(),
                        "distance": dist_km,
                        "stale": stale,
                        "active": (cs.upper() == (self.client._current_rx_cs or "").upper())
                    })

                status = {
                    "callsign": cfg.callsign.upper(),
                    "profile": getattr(self.client.cfg, "radio_profile", "PRC152").upper(),
                    "online": engine._running,
                    ...
                    "vox": cfg.vox_enabled,
                    "tx_active": engine._tx_active,
                    "tx_level": float(getattr(engine, '_tx_level', 0.0)),
                    "rx_active": bool(self.client._current_rx_cs),
                    "rx_level": float(getattr(engine, 'rx_audio_level', 0.0)),
                    "rx_callsign": self.client._current_rx_cs or "",
                    "signal_strength": engine._rx_signal,
                    "mgrs": getattr(self.client, '_last_mgrs', "00A AA 0000 0000"),
                    "gps_fix": True,
                    "tak_online": bool(engine._tak and engine._tak._running),
                    "rtt": engine._last_rtt_ms,
                    "battery": 88.0,
                    "roster_count": getattr(self.client, "_roster_count", 0),
                    "tab_index": getattr(self.client, "_current_tab_index", 0),
                    "jammed": bool(engine._effects.distortion > 0.5 or engine._effects.noise_floor > 0.5),
                    "channels": channels_map,
                    "active_channels": active_channels,
                    "members": members_list,
                    "nav_bearing": nav_bearing,
                    "nav_range_km": nav_range_km,
                    "nav_name": self.client._nav_name
                }
    ...
    def do_POST(self):
        ...
        if self.path == '/api/ptt':
            ...
            if state == "on":
                self.client.engine.ptt_on()
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "ptt": "on"}).encode('utf-8'))
            elif state == "off":
                self.client.engine.ptt_off()
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "ptt": "off"}).encode('utf-8'))
            else:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "Invalid PTT state"}).encode('utf-8'))

        elif self.path == '/api/channel':
            ch = payload.get("channel")
            action = payload.get("action")
            
            assigned = self.client.engine._assigned_channels
            valid_channels = sorted(self.client.mgr.channels.keys())
            if assigned is not None:
                valid_channels = [c for c in valid_channels if c in assigned]

            if not valid_channels:
                self._set_headers(500)
                self.wfile.write(json.dumps({"error": "No assigned channels"}).encode('utf-8'))
                return

            curr_channel = self.client.cfg.current_channel
            curr_idx = 0
            if curr_channel in valid_channels:
                curr_idx = valid_channels.index(curr_channel)

            target_channel = None
            if action == "next":
                target_channel = valid_channels[(curr_idx + 1) % len(valid_channels)]
            elif action == "prev":
                target_channel = valid_channels[(curr_idx - 1) % len(valid_channels)]
            elif ch is not None:
                try:
                    ch_val = int(ch)
                    if ch_val in valid_channels:
                        target_channel = ch_val
                except ValueError:
                    pass

            if target_channel is not None:
                self.client.engine.change_channel(target_channel)
                self.client.channels_changed_signal.emit()
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "channel": target_channel}).encode('utf-8'))
            else:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "Invalid channel or action"}).encode('utf-8'))
            ...
            if action == "mute":
                ...
                target_vol = getattr(self.client, '_last_unmute_vol', 0.8)
            elif level is not None:
                try:
                    target_vol = float(level)
                    if target_vol < 0.0: target_vol = 0.0
                    if target_vol > 2.0: target_vol = 2.0
                except ValueError:
                    pass

            if target_vol is not None:
                if target_vol > 0.0:
                    self.client._last_unmute_vol = target_vol
                self.client.volume_changed_signal.emit(target_vol)
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "volume": target_vol}).encode('utf-8'))
            else:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "Invalid volume level or action"}).encode('utf-8'))

        elif self.path == '/api/vox':
            state = payload.get("state")
            if state is not None and isinstance(state, bool):
                self.client.vox_changed_signal.emit(state)
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "vox": state}).encode('utf-8'))
            else:
                self._set_headers(400)
                self.wfile.write(json.dumps({"error": "Invalid VOX state"}).encode('utf-8'))

        elif self.path == '/api/mon':
            ch_id = payload.get("channel")
            state = payload.get("state")
            if ch_id is not None and state is not None:
                try:
                    ch_val = int(ch_id)
                    if ch_val in self.client.mgr.channels:
                        if state:
                            self.client.engine.monitor_channel(ch_val)
                        else:
                            self.client.engine.unmonitor_channel(ch_val)
                        self.client.channels_changed_signal.emit()
                        self._set_headers(200)
                        self.wfile.write(json.dumps({"status": "success", "channel": ch_val, "monitored": state}).encode('utf-8'))
                        return
                except ValueError:
                    pass
            self._set_headers(400)
            self.wfile.write(json.dumps({"error": "Invalid channel or state"}).encode('utf-8'))

        elif self.path == '/api/tab':
            tab_val = payload.get("tab")
            if tab_val is not None:
                try:
                    tab_idx = int(tab_val)
                    if 0 <= tab_idx <= 4:
                        self.client.tab_changed_signal.emit(tab_idx)
                        self._set_headers(200)
                        self.wfile.write(json.dumps({"status": "success", "tab": tab_idx}).encode('utf-8'))
                        return
                except ValueError:
                    pass
            self._set_headers(400)
            self.wfile.write(json.dumps({"error": "Invalid tab index"}).encode('utf-8'))

        elif self.path == '/api/comsec':
            cfg = self.client.cfg
            curr_channel = cfg.current_channel
            if curr_channel in self.client.mgr.channels:
                ch_info = self.client.mgr.channels[curr_channel]
                currently_encrypted = ch_info.encrypted
                if currently_encrypted:
                    ch_info.passphrase = ""
                    ch_info.comsec_mode = "Plain"
                else:
                    import configparser
                    original_passphrase = "SECURE"
                    original_mode = "AES-256"
                    try:
                        parser = configparser.ConfigParser()
                        parser.read(str(self.client.mgr.path), encoding="utf-8")
                        sect = f"channel.{curr_channel}"
                        if parser.has_section(sect):
                            p = parser.get(sect, "passphrase", fallback="").strip()
                            if p:
                                original_passphrase = p
                            m = parser.get(sect, "comsec_mode", fallback="").strip()
                            if m and m != "Plain":
                                original_mode = m
                    except Exception:
                        pass
                    
                    ch_info.passphrase = original_passphrase
                    ch_info.comsec_mode = original_mode
                
                self.client.engine.rebuild_codec()
                self.client.channels_changed_signal.emit()
                ...
                self.wfile.write(json.dumps({
                    ...
                    "encrypted": not currently_encrypted,
                    "comsec_mode": ch_info.comsec_mode
                }).encode('utf-8'))
                return
            self._set_headers(400)
            self.wfile.write(json.dumps({"error": "No active channel"}).encode('utf-8'))

        elif self.path == '/api/log':
            msg = payload.get("message")
            stack = payload.get("stack")
            line = payload.get("line")
            err_msg = f"--- STREAM DECK JS ERROR --- {msg} (line {line})\n{stack}\n"
            logging.error(err_msg)
            try:
                with open("js_error.log", "a", encoding="utf-8") as f:
        ...
        elif self.path == '/api/member/highlight':
            cs = payload.get("callsign")
            if cs:
                self.client._highlighted_member = cs.upper()
                self.client.tab_changed_signal.emit(3) # Snap to map tab
                self.client.plotter.update()
                self._set_headers(200)
                self.wfile.write(json.dumps({"status": "success", "highlighted": cs}).encode('utf-8'))
                return
            self._set_headers(400)
            self.wfile.write(json.dumps({"error": "Callsign required"}).encode('utf-8'))
        elif self.path == '/api/nav/target':
            lat = payload.get("lat")
            lon = payload.get("lon")
            name = payload.get("name", "WAYPOINT")
            if lat is not None and lon is not None:
                try:
                    self.client._nav_lat = float(lat)
                    self.client._nav_lon = float(lon)
                    self.client._nav_name = str(name)
                    self._set_headers(200)
                    self.wfile.write(json.dumps({
                        "status": "success", 
                        "lat": self.client._nav_lat, 
                        "lon": self.client._nav_lon, 
                        "name": self.client._nav_name
                    }).encode('utf-8'))
                    return
                except ValueError:
                    pass
            self._set_headers(400)
            self.wfile.write(json.dumps({"error": "Invalid coordinates"}).encode('utf-8'))
        else:
            self._set_headers(404)
            self.wfile.write(json.dumps({"error": "Not Found"}).encode('utf-8'))
...
class AetherClient(QMainWindow):
    ...
    assigned_channels_signal = Signal(object)
    activity_signal = Signal(int, bool)
    channels_changed_signal = Signal()
    volume_changed_signal = Signal(float)
    vox_changed_signal = Signal(bool)
    tab_changed_signal = Signal(int)
    ...
    def __init__(self, engine: RadioEngine, config_mgr: ConfigManager):
        ...
        self._ptt_hotkey_active = False   # Bug 1: guard against OS key-repeat sidetone
        self._last_intended_channel = None  # Bug 2: track last server-pushed intended channel
        self._roster_count = 0
        self._current_tab_index = 0
        self._highlighted_member = ""
        self._nav_lat = 0.0
        self._nav_lon = 0.0
        self._nav_name = ""
        ...
        # Set application window icon using the bundled client.png
        from radio_config import resource_path as _rp
        _icon_path = _rp("client.png")
        ...
        # Start server audio config polling thread
        ...
        # Start local REST API if enabled
        self._last_unmute_vol = self.cfg.output_volume if self.cfg.output_volume > 0 else 0.8
        if getattr(self.cfg, "enable_local_api", True):
    ...
    def _init_ui(self):
        ...
        self.lbl_eccm_badge = QLabel("FH")
        self.lbl_eccm_badge.setStyleSheet(f"""
            QLabel {{
                background-color: {C_BORDER};
                color: {C_ACCENT};
        ...
        """)
        self.lbl_eccm_badge.setVisible(False)
        top_info.addWidget(self.lbl_eccm_badge)
        top_info.addSpacing(10)
        ...
        # Remove keyboard focus from all buttons to prevent spacebar (PTT) activating them
    ...
    def _connect_engine(self):
        ...
        self.channels_changed_signal.connect(self._populate_channel_list)
        self.volume_changed_signal.connect(self._update_volume_ui)
        self.vox_changed_signal.connect(self._update_vox_ui)
        self.tab_changed_signal.connect(self._switch_tab)

    def _on_activity_received(self, channel_id, active):
        if not hasattr(self, '_channel_activity_stamps'):
            self._channel_activity_stamps = {}
        if active:
            self._channel_activity_stamps[channel_id] = time.time()
        ...
        try:
            ...
            def on_press(key):
                try:
                    ...
                    if kname == "space": kname = "space"
                    if kname == self.cfg.ptt_key:
                        # Bug 1 fix: only trigger on first physical press, not OS key-repeats
                        if not self._ptt_hotkey_active:
                            self._ptt_hotkey_active = True
                            self.engine.ptt_on()
                except: pass
            ...
            def on_release(key):
                try:
                    kname = key.char if hasattr(key, 'char') else key.name
                    if kname == self.cfg.ptt_key:
                        self._ptt_hotkey_active = False  # Bug 1 fix: reset so next press works
                        self.engine.ptt_off()
                except: pass
    ...
    def _update_members(self, members):
        self._roster_count = len(members)
    ...
    def _populate_channel_list(self, _ignored=None):
        ...
        for ch_id in sorted(self.mgr.channels.keys()):
            # Filter: if assigned is a set (even empty), enforce it. None means open access.
            if assigned is not None and ch_id not in assigned:
                continue
    ...
    def _start_local_api(self):
        port = getattr(self.cfg, "local_api_port", 8895)
        def run_server():
            class CustomHandler(ClientAPIHandler):
                client = self
            try:
                self._api_server = ThreadingHTTPServer(("127.0.0.1", port), CustomHandler)
                logging.info(f"Local REST API server running on http://127.0.0.1:{port}")
                self._api_server.serve_forever()
            except Exception as e:
                logging.error(f"Failed to start local REST API server: {e}")
        
        self._api_thread = threading.Thread(target=run_server, daemon=True, name="local-api-server")
        self._api_thread.start()

    @Slot(float)
    def _update_volume_ui(self, vol):
        self.slider_vol.blockSignals(True)
        self.slider_vol.setValue(int(vol * 100))
        self.slider_vol.blockSignals(False)
        self.cfg.output_volume = vol
        self.engine._effects.update(volume=vol)

    @Slot(bool)
    def _update_vox_ui(self, enabled):
        self.btn_vox.blockSignals(True)
        self.btn_vox.setChecked(enabled)
        self.btn_vox.blockSignals(False)
        self.cfg.vox_enabled = enabled
        self._update_vox_style(enabled)

    def _audio_config_poll_loop(self):
        """Poll server every 5 seconds for global audio and DSP settings."""
        while getattr(self, '_polling_active', True):
            self._apply_server_audio_config()
            time.sleep(5)

    def _apply_server_audio_config(self) -> None:
        """Fetch server config and update the engine's audio effects live."""
        try:
            import urllib.request
            import urllib.parse
            import json
            
            web_port = getattr(self.cfg, 'web_admin_port', 8890)
            host     = getattr(self.cfg, 'server_host', '127.0.0.1')
            config_url  = f"http://{host}:{web_port}/api/config"
            status_url  = f"http://{host}:{web_port}/api/clients/{urllib.parse.quote(self.cfg.callsign)}"
            
            # 1. Fetch General Audio Config
            with urllib.request.urlopen(config_url, timeout=2) as resp:
                srv_cfg = json.loads(resp.read().decode())
            
            # Update local effects engine live
            self.engine._effects.update(
                squelch             = srv_cfg.get('squelch', 0.0),
                volume              = self.cfg.output_volume, # Use local volume slider setting
                distortion          = srv_cfg.get('distortion', 0.0),
                noise_floor         = srv_cfg.get('noise_floor', 0.0),
                crackle_lvl         = srv_cfg.get('crackle_lvl', 0.0),
                ptt_click_enabled   = srv_cfg.get('ptt_click_enabled', False),
                ptt_type            = srv_cfg.get('ptt_type', 'analog'),
                tx_highpass_enabled = srv_cfg.get('tx_highpass_enabled', False),
                tx_preemph_enabled  = srv_cfg.get('tx_preemph_enabled', False),
                squelch_tail_enabled = srv_cfg.get('squelch_tail_enabled', False),
                bp_low_hz           = srv_cfg.get('bp_low_hz', 120),
                bp_high_hz          = srv_cfg.get('bp_high_hz', 7000),
                
                # Atmosphere & Weather
                weather_enabled         = srv_cfg.get('weather_enabled', False),
                weather_severity        = srv_cfg.get('weather_severity', 0.0),
                weather_type            = srv_cfg.get('weather_type', 'rain'),
                weather_humidity        = srv_cfg.get('weather_humidity', 0.0),
                temp_inversion_enabled  = srv_cfg.get('temp_inversion_enabled', False),
                temp_inversion_strength = srv_cfg.get('temp_inversion_strength', 0.0),
                temp_inversion_echo_ms  = srv_cfg.get('temp_inversion_echo_ms', 10.0),
                iono_enabled            = srv_cfg.get('iono_enabled', False),
                iono_condition          = srv_cfg.get('iono_condition', 'quiet'),
                iono_fading             = srv_cfg.get('iono_fading', 0.0),
                iono_flutter            = srv_cfg.get('iono_flutter', 0.0),
                iono_absorption         = srv_cfg.get('iono_absorption', 0.0),
                
                # Day / Night
                day_night_enabled       = srv_cfg.get('day_night_enabled', False),
                day_night_hour          = srv_cfg.get('day_night_hour', 12.0),
                day_night_auto          = srv_cfg.get('day_night_auto', False),
                day_night_latitude      = srv_cfg.get('day_night_latitude', 0.0),
                
                # Day/Night/EMI
                emi_enabled             = srv_cfg.get('emi_enabled', False),
                emi_level               = srv_cfg.get('emi_level', 0.0),
                emi_type                = srv_cfg.get('emi_type', 'white'),
            )
            
            # Sync local config so UI / encoder knows the state
            self.cfg.ptt_type = srv_cfg.get('ptt_type', self.cfg.ptt_type)
            self.cfg.squelch_tail_enabled = srv_cfg.get('squelch_tail_enabled', self.cfg.squelch_tail_enabled)
            
            # Sync TAK Config dynamically from the server (Ground Truth)
            srv_tak_enabled = srv_cfg.get('tak_enabled')
            if srv_tak_enabled is not None:
                srv_tak_url = srv_cfg.get('tak_server_url', self.cfg.tak_server_url)
                if (srv_tak_enabled != self.cfg.tak_enabled) or (srv_tak_url != self.cfg.tak_server_url):
                    logging.info(f"TAK config synced from server: enabled={srv_tak_enabled}")
                    self.cfg.tak_enabled = srv_tak_enabled
                    self.cfg.tak_server_url = srv_tak_url
                    self.engine.restart_tak()
            
            # 2. Fetch Client Specific Config (Profile, Channels, Intended push)
            try:
                ...
                if cli_info:
                    # Sync Profile name
                    if "radio_profile" in cli_info:
                        new_prof = cli_info["radio_profile"]
                        if new_prof != self.cfg.radio_profile:
                            logging.info(f"Server changed profile to: {new_prof}")
                            self.cfg.radio_profile = new_prof
                            
                    # Sync Position (Allows admin to drag callsigns on map and update the client)
                    if "lat" in cli_info and "lon" in cli_info:
                        s_lat = float(cli_info.get("lat", 0.0))
                        s_lon = float(cli_info.get("lon", 0.0))
                        s_alt = float(cli_info.get("alt", 0.0))
                        c_lat, c_lon, _ = self.mgr.get_position()
                        if abs(s_lat - c_lat) > 0.00001 or abs(s_lon - c_lon) > 0.00001:
                            self.mgr.set_position(s_lat, s_lon, s_alt)
                            
                    # Switch intended channel if pushed — Bug 2 fix:
                    # Only act when the server's intended_channel value itself changes,
                    # not simply because the user has manually moved to a different channel.
                    # This prevents the poll loop from reverting manual user channel changes.
                    if "intended_channel" in cli_info:
                        new_ch = cli_info["intended_channel"]
                        if new_ch is not None and new_ch != self._last_intended_channel:
                            # Server genuinely changed its mind about the intended channel
                            self._last_intended_channel = new_ch
                            if new_ch != self.cfg.current_channel:
                                logging.info(f"Server pushed NEW intended channel: CH{new_ch:02d}")
                                self.engine.change_channel(new_ch)
                                self.channels_changed_signal.emit()
                        elif new_ch is None and self._last_intended_channel is not None:
                            # Server cleared the intended channel assignment
                            self._last_intended_channel = None
                            
                    # Update Assigned Channels
                    if "assigned_channels" in cli_info:
                        assigned_chs = cli_info.get("assigned_channels")
                        if assigned_chs is None:
                            new_assigned = None
                        else:
                            new_assigned = set(int(c) for c in assigned_chs if c is not None and int(c) >= 0)
                            
                        if new_assigned != self.engine._assigned_channels:
                            logging.info(f"Assigned channels updated: {new_assigned}")
                            self.engine._assigned_channels = new_assigned
                            # Emit list if it's a set, else None
                            emit_val = list(new_assigned) if new_assigned is not None else None
                            self.assigned_channels_signal.emit(emit_val)
                            
                            # Switch channel if current channel is no longer assigned
                            if new_assigned and self.cfg.current_channel not in new_assigned:
                                logging.info("Current channel no longer assigned. Auto-switching...")
                                self.engine.change_channel(min(new_assigned))
                                self.channels_changed_signal.emit()
                                 
                    # Jamming Sync
                    is_jammed = cli_info.get("jammed", False)
                    if is_jammed:
                        self.engine._effects.update(
                            noise_floor = 0.8,
                            distortion  = 0.6
                ...
                logging.warning(f"Client status sync failed: {inner_e}")
                
        except Exception as e:
            logging.exception(f"Audio config sync failed: {e}")
    ...
    def _update_telemetry(self):
        ...
        if ch_def:
            ...
            # Update COMSEC label based on channel setting
            comsec = getattr(ch_def, 'comsec_mode', 'Plain') or 'Plain'
            if comsec != "Plain":
                self.lbl_enc.setText(comsec.upper())
                self.lbl_enc.setVisible(True)
            else:
                self.lbl_enc.setVisible(False)
            
            # Update ECCM Badge based on channel setting
            eccm = getattr(ch_def, 'eccm_mode', 'None') or 'None'
            if eccm != "None":
                self.lbl_eccm_badge.setText("FH" if eccm == "SINCGARS" else "HQ")
                self.lbl_eccm_badge.setVisible(True)
                
                is_active_comm = self.engine._tx_active or bool(self._current_rx_cs)
                if is_active_comm:
                    blink = int(time.time() * 4) % 2 == 0
                    if blink:
                        self.lbl_eccm_badge.setStyleSheet(f"background-color: {C_PTT}; color: white; border: 1px solid #f87171; border-radius: 4px; padding: 1px 4px; font-family: 'JetBrains Mono'; font-size: 9px; font-weight: bold;")
                    else:
                        self.lbl_eccm_badge.setStyleSheet(f"background-color: transparent; color: {C_PTT}; border: 1px solid {C_PTT}; border-radius: 4px; padding: 1px 4px; font-family: 'JetBrains Mono'; font-size: 9px; font-weight: bold;")
                else:
                    # Solid idle state
                    self.lbl_eccm_badge.setStyleSheet(f"background-color: {C_BORDER}; color: {C_ACCENT}; border: 1px solid {C_ACCENT_DIM}; border-radius: 4px; padding: 1px 4px; font-family: 'JetBrains Mono'; font-size: 9px; font-weight: bold;")
            else:
                self.lbl_eccm_badge.setVisible(False)

            # Frequency Label Micro-Flicker during active communications on an ECCM channel
            is_active_comm = self.engine._tx_active or bool(self._current_rx_cs)
            if eccm != "None" and is_active_comm:
                import random
                d1 = random.randint(0, 9)
                d2 = random.randint(0, 9)
                self.freq_label.setText(f"{freq:07.1f}{d1}{d2} MHz")
            else:
            ...
            mode_suffix = "JAMMED" if is_sim else "CLEAR"
        ...
        if self.engine._tx_active:
            ...
            effects = self.engine._effects
            is_sim = effects.distortion > 0.0 or effects.noise_floor > 0.0
            if is_sim:
                self.waveform.update_level(rx_level, True)
                self.lbl_tx_mode.setText("JAMMED")
                self.lbl_tx_mode.setStyleSheet(f"color: {C_PTT}; font-family: 'JetBrains Mono'; font-weight: bold;")
                try: self.lcd_panel.graphicsEffect().setColor(QColor(239, 68, 68, 45))
                except: pass
            else:
...
# ── PTT Spacebar Focus Intercept ────────────────────────────────────────────
class SpacebarFilter(QObject):
    """
    Global Qt event filter that consumes spacebar KeyPress/KeyRelease events
    unless the focused widget is a text-entry field (QLineEdit, QTextEdit,
    QPlainTextEdit).  Prevents a focused channel button from being 'clicked'
    by the spacebar PTT key.
    """
    def eventFilter(self, obj, event):
        if event.type() in (QEvent.KeyPress, QEvent.KeyRelease):
            if event.key() == Qt.Key_Space:
                ...
                return True        # Consume — block button activation
...
def main():
    ...
    # Feature 1: Install global spacebar focus intercept filter
    spacebar_filter = SpacebarFilter(app)
    ...
    if args.server:
        ...
        if port_str:
            port = int(port_str)
            # When started remotely or explicitly on port 8890, the port is the web admin port
            if args.remote or port == 8890:
                mgr.config.web_admin_port = port
            else:
    ...
    # Silently sync channels and server config on startup if remote or callsign specified
    if args.remote or args.callsign:
        host = mgr.config.server_host
        port = mgr.config.web_admin_port
        logging.info(f"Syncing channels plan and config from server http://{host}:{port}...")
        try:
            # 1. Fetch server config to sync UDP ports
            url_cfg = f"http://{host}:{port}/api/config"
            with urllib.request.urlopen(url_cfg, timeout=4) as r:
                srv_cfg = json.loads(r.read().decode())
                if "server_port" in srv_cfg:
                    mgr.config.server_port = int(srv_cfg["server_port"])
                    logging.info(f"Synced UDP server port: {mgr.config.server_port}")
                if "server_ctrl_port" in srv_cfg:
                    mgr.config.server_ctrl_port = int(srv_cfg["server_ctrl_port"])
                if "server_voice_port" in srv_cfg:
                    mgr.config.server_voice_port = int(srv_cfg["server_voice_port"])
            
            # 2. Fetch server channels plan
            from radio_config import ChannelDef
            ...
            with urllib.request.urlopen(url_plan, timeout=4) as r:
                ...
                if channels:
                    ...
                    for c in channels:
                        ch_id = int(c["id"])
                        mgr.channels[ch_id] = ChannelDef.from_dict(c)
                    mgr.save()
                    logging.info(f"Successfully synced {len(mgr.channels)} channels from server.")
        except Exception as e:
            logging.warning(f"Failed to silently sync channels/config from server: {e}")
